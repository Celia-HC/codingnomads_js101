...
//1. Output the total amount of expenses for 2017 and 2018.
let totalExpenses = 0;
function yearExpenses(yearDeclared) {
  expenses.forEach((element, index) => {
	if (index !== 0) {
		let date = new Date(element[0]);
		let year = date.getFullYear();
    if (element[3]<0)
		if (year === yearDeclared) {
			totalExpenses += element[3];
		}
	}
})
return (`Expenses for year ${yearDeclared} : ${totalExpenses.toFixed(2)}`)}
console.log(yearExpenses(2017));
console.log(yearExpenses(2018));
...
expenses.forEach(function (element, index) {
	if (index !== 0) {
		// Ignore the first row which contains column names
		if (element[2] === "Groceries") {
			// Category is at index 2
			totalGroceries += element[3]; // Amount is at index 3
		}
	}
});

console.log("Total amount paid for Groceries:", totalGroceries);
...
let total = 0;
function totalByAccount (account) {
  expenses.forEach((element, index) => {
    if (index !== 0) {
    if (element[4] === account) {
      total += element[3];
    }}
  })
  return(total)
